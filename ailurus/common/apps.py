...
class TsingHuaTeXTemplate(download_one_file) : # released under GPL
...
class FFAutoProxy(ff_extension):
    'AutoProxy'
...
class FFChromeTheme_3_0(ff_extension):
...
class FFChromeTheme_3_5(ff_extension): 
...
class FFCleanHide(ff_extension):
...
class FFDownloadStatusBar(ff_extension): 
    'DownloadStatusBar'
...
class FFDownThemAll(ff_extension):
...
class FFEasyDragToGo(ff_extension):
...
class FFFireGesture(ff_extension):
...
class FFFoxyProxy(ff_extension):
...
class FFGreaseMonkey(ff_extension):
...
class FFLiveHTTPHeaders(ff_extension):
...
class FFTabMixLite(ff_extension):
...
class FFTamperData(ff_extension):
...
class FFUserAgentSwitcher(ff_extension):
...
class FFViewSourceChart(ff_extension):
...
class FFWeaveSync35(ff_extension):
...
class FFWebDeveloper(ff_extension):
...
class FFYetAnotherSmoothScrolling(ff_extension):
